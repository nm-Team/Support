...
1. 若 Telegram 客户端提示“此用户隐藏了自己的账号身份”，则该消息是由仿冒 nmBot 发送的。请注意，nmBot 技术上无法隐藏自己的账号身份。若没有看到此提示，转到下一步。
1. 检查消息发送者的用户名。nmBot 发送的消息总是来自用户名 `@nmnmfunbot` 的账号，仿冒者则无法使用此用户名。若消息发送者的用户名不是 `@nmnmfunbot`，或没有用户名，则该消息是由仿冒者发送的。请仔细确认拼写，因为仿冒者可能会使用十分相似的用户名。
